...
Use this file only when the user asks for deep explanation, source-backed verification, stronger interview prep, or bullet optimization. The codewiki source files are in `references/wiki/` (23 files). Locate the relevant `Wxx` section first, then read only the matching file.
...
3. Read only the matching file(s) from `references/wiki/`.
