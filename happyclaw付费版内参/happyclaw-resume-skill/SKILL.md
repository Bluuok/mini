---
name: happyclaw-resume-skill
description: Generate HappyClaw multi-user Agent workstation and enterprise digital employee resume projects, scenario-align resumes to target company JDs, predict interview Q&A, and deep-dive into technical points. Use when the user asks to package HappyClaw as a resume project, adapt HappyClaw to a specific job/company scenario, prepare interview Q&A predictions, or deep-dive into a HappyClaw technical point using the bundled codewiki.
---
...
Use this workflow to help a user turn HappyClaw into a credible multi-user Agent workstation and enterprise digital employee resume project and interview prep pack.

1. Read `references/user-intake.md` first when the user's background, target role, mastery level, or risk preference is missing. Ask only the missing high-impact questions.
2. Read `references/combo-plans.md` to choose a default plan for the user's target role.
...
4. Read `references/resume-points.md` to select only 4-5 `Rxx` points the user can plausibly explain. Review the per-point Scope Boundary field.
...
6. Read `references/interview-qa.md` and return the corresponding `Qxx.x` prep list. Structure answers in four segments: problem solved, HappyClaw implementation, design tradeoff, limitation and extension.
7. If the user asks to deep-dive, verify, strengthen, or explain a technical point, read `references/codewiki-index.md`, then load only the relevant file from `references/wiki/`.

## Scenario Alignment Path (Step 1a)

When the user provides a JD and/or company background, reframe HappyClaw to that company's business scenario using these steps:
...
2. **Load capabilities**: Read `resume-points.md`, `combo-plans.md`, `interview-qa.md`, and `user-intake.md`. If the user hasn't selected a plan, default to Plan H (`R01 R07 R11 R15 R23`). Downgrade high-risk points (R04/R16/R23) per `user-intake.md` rules (e.g., R23->R19).
3. **Map scenario to capabilities**: Use "Use when" in combo-plans.md and Scope Boundary in resume-points.md to match each business scenario to HappyClaw capabilities.
4. **Reframe descriptions** (core rewrite): Transform "I did X" into "To solve [company scenario]'s [pain point], did X, achieving [business effect]". Use precise numbers only when the JD provides them; otherwise use "target / expected". **Rewritten bullets must keep the four-segment resume structure: 针对[公司场景业务问题]，基于[技术]，设计了[工程方案]，实现了[验证效果] — replace the business-problem slot with the JD scenario's pain point; verification effects stay qualitative (target / expected) unless the JD gives numbers.**
   - **Frontier topic mapping**: JD keywords 评测/评估/质检 -> R25（行为评测底座）；技能/能力治理/进化/经验复用 -> R26（能力治理与进化）；安全/合规/权限/风险/审计 -> R27（纵深防御）。前沿点输出时必须带【项目实现】/【前沿认知】标注：平台真实能力进简历 bullet，论文证据（SkillsBench +16.6pp、AI Control 15%→92%、26.1% 技能漏洞率）只进面试 talking points，严禁写进 bullet。
   - **Capability attribution guardrail**: Reframing only changes the narrative frame, not the capability level. Each R point may only claim capabilities within its Scope Boundary. Non-native capabilities (business APIs, external system integration) must be labeled with their R point attribution and whether they are self-built. Common overreach patterns: relabeling load balancing as task-aware routing, relabeling Web login auth as Agent runtime identity, relabeling declarative prompt as executable orchestration engine.
5. **IM bridge (landing enhancement)**: HappyClaw has native 7-channel IM (飞书/Telegram/QQ/钉钉/微信/Discord/WhatsApp). WeChat channel is iLink/P2P only, not WeCom. WeCom requires cc-connect wecom adapter. Slack and other uncovered platforms require cc-connect. Each channel has different capabilities; do not write "all channels functionally identical".
...
7. **Post-downgrade positioning recheck**: If high-risk points were downgraded in step 2 (especially R23->R19), verify that project name positioning terms ("multi-tenant", "runtime security", "security baseline") still have selected R-point support. Remove or swap any unsupported terms. Per `combo-plans.md` Plan H downgrade positioning rules, dropping "multi-tenant" and shifting to "multi-user Agent workstation + digital employee" is required.
...
- Predict company-specific questions based on company background (e.g., cross-border company may ask about data compliance; if JD mentions RAG, predict RAG-related questions that HappyClaw cannot answer since it has no RAG capability).
...
- For each, give a concrete downgrade path (e.g., "R23 multi-tenant isolation: if you can't explain canExecuteOnHost real-time authorization, downgrade to R19 Web auth and prepare a 'understood design layer' talking point").
...
- **Capability attribution**: each bullet may only claim capabilities within the selected R point's Scope Boundary (see `resume-points.md`). Do not name components belonging to unselected R points (e.g., IM Owner Gate belongs to R20, not R19; canExecuteOnHost belongs to R23, not R19). Do not relabel load balancing as task-aware routing, or Web login auth as Agent runtime identity. Do not claim RAG/vector retrieval capabilities-HappyClaw R points are platform-layer only.
...
- Position HappyClaw primarily as a multi-user Agent workstation and enterprise digital employee platform, not as a generic chat app.

## Default Selection Logic

- Beginner or conservative: `R02`, `R13`, `R18`, `R21`, `R24`.
- Agent core architecture: `R01`, `R03`, `R05`, `R06`, `R15`.
- Multi-channel IM platform / digital employee: `R07`, `R08`, `R09`, `R10`, `R11`.
- Data and scheduling engineering: `R13`, `R14`, `R09`, `R20`, `R08`.
- Frontend and real-time experience: `R21`, `R22`, `R06`, `R18`, `R17`.
- Agent capability governance / MCP ecosystem: `R15`, `R16`, `R17`, `R01`, `R03`.
- Enterprise digital employee (full): `R07`, `R14`, `R15`, `R20`, `R23`.
- Unsure or paid-member default: `R01`, `R07`, `R11`, `R15`, `R23`.
...
For normal resume generation, do not load the full codewiki files. For technical deep dives, use `references/codewiki-index.md` to locate the relevant `Wxx` section and then read only the matching file from `references/wiki/`.
