# HappyClaw 完整工作流（原 skill 原样迁移）

> 这是原 `happyclaw-resume-skill` 的完整工作流，原样保留：Mode Detection、Resume 工作流（简历大致描述 → 4-5 条要点）、Scenario 场景化对齐（Step 1a）、Q&A 预测（Step 1b）、8 模块故事生成（Step 1c）、输出规则、默认选点逻辑、Deep-Dive。
...
6. Read `./interview-qa.md` and return the corresponding `Qxx.x` prep list. Structure answers in four segments: problem solved, HappyClaw implementation, design tradeoff, limitation and extension.
...
For normal resume generation, do not load the full codewiki files. For technical deep dives, use `./codewiki-index.md` to locate the relevant `Wxx` section and then read only the matching runtime codewiki file (see `../project-router.md` for runtime location).
