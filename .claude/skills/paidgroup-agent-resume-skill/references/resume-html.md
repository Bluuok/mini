...
本 skill 的模板资源固定位于 `../assets/`（本文件在 `references/` 下，上一级即 skill 根目录），包含：
...
如果 `../assets/` 不完整或缺失（被单独复制、删减），必须显式说明“模板资源未随 skill 一起安装”，并启用后备方案：
...
3. 经历属于三项目时，先按 `project-router.md` 用项目 refs 准备内容（R 点、四段式 bullet），再排版。
4. 按“模板选择矩阵”选择模板、页数和风格；如果资源缺失，启用后备模板并说明限制。
5. 输出 HTML 文件，保持文字和布局可编辑；工具栏放在页面外并在打印时隐藏。
6. 把 print-preview 作为导出前强制 QA：打开或模拟打印预览，检查分页、双栏、照片比例、长文本溢出、中文字体、打印边距和 A4 纸张效果；发现问题先修复 HTML/CSS 再交付。
7. 用户需要 PDF 时，按“PDF 导出验收标准”说明浏览器打印设置，或按用户要求生成 PDF；不要用截图替代可编辑源文件。
...
三项目岗位对应：AI Agent / LLM 应用开发 / FDE 方向按“数据分析/算法/AI 应用”行处理，项目经历与证据优先；后端/CLI 工具链方向按“后端开发”行处理。
