# MiniCode 完整工作流（原 skill 原样迁移）

> 这是原 `minicode-resume-skill` 的完整工作流，原样保留：Mode Detection、Resume 工作流（简历大致描述 → 4-5 条要点）、Scenario 场景化对齐（Step 1a）、Q&A 预测（Step 1b）、8 模块故事生成（Step 1c）、输出规则、默认选点逻辑、Deep-Dive。
...
1. Read `./user-intake.md` first when the user's background, target role, or mastery level is missing. Ask only the missing high-impact questions.
2. Read `./combo-plans.md` to pick a default plan for the user's target role.
...
3. Read `./resume-points.md` to select 4-5 `Rxx` points the user can plausibly explain. Review the per-point "What to Explain", "Interview Risk", and "Scope Boundary" fields before finalizing.
4. Generate a resume project entry: **one-sentence project summary (项目简介一句话)**, tech stack, and 4-5 bullets. **Every bullet MUST follow the four-segment structure: 针对[业务问题]，基于[技术]，设计了[工程方案]，实现了[验证效果]。** Use the pre-written bullets from `./resume-points.md` verbatim (already in this format) for consistent technical precision. Adapt wording only if the user has domain-specific framing needs; the four-segment structure must be preserved.
5. Read `./interview-qa.md` and return the corresponding `Qxx.x` prep list. When returning Q&A, instruct the user to structure answers in four segments: (1) what problem this solves, (2) how MiniCode implements it, (3) why not a simpler or more complex approach, (4) limitations and extensibility.
6. If the user asks to deep-dive, verify, strengthen, or explain a specific technology point, read `./codewiki-index.md` and then locate the matching runtime codewiki file (see `../project-router.md` for runtime location).
...
For normal resume generation, do not load the full CodeWiki. For deep technical questions, use `./codewiki-index.md` to find the section, then read the matching runtime codewiki file (see `../project-router.md` for runtime location).
