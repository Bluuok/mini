...
Treat each plan as a legacy R-point candidate pool and risk guide, not a fixed starting set or fixed bullet quota. Build the Agent Project Model first, then select at least 4 non-duplicative Artifacts across plans based on architecture layer, evidence, ownership, and job relevance. Do not copy a full plan by default.
